...
public class HytaleInsightsUpdate {
    public static String checkForUpdate(){
        try{
            URL url = new URL("https://api.mermaids.dev/versions/hytale-insights/release/");
            ...
            HytaleInsights.LOGGER.atWarning().log("Exception with HytaleInsightsUpdate, possible invalid url : " + e.toString());

            return HytaleInsights.getVersion();
        }
    ...
    public static void sendUpdateMessage(Type type){
        if(Type.StartUp.getValue() == type.getValue()) {
            ...
            if(!HytaleInsights.getVersion().equalsIgnoreCase(recentVersion)) {
                HytaleInsights.LOGGER.atInfo().log("= =- -=- -=- -=- -=- -=- -=- -=- -= =");
                HytaleInsights.LOGGER.atInfo().log("The Hytale Insights mod version is outdated, Hytale Insights has released v" + recentVersion + ".");
            }
    ...
    public static void sendUpdateMessage(@Nullable Player player, boolean sendToPlayer){
        String recentVersion = checkForUpdate();
        if(!HytaleInsights.getVersion().equalsIgnoreCase(recentVersion)){
            String translationId = "server.updateChecker.hytaleinsights.release.message";
            Message versionMessage = Message.translation(translationId).param("version", recentVersion);

            HytaleInsights.LOGGER.atInfo().log(versionMessage.getAnsiMessage());

            if(sendToPlayer && player != null) {
                if (player.hasPermission("*") && HytaleInsights.getConfig().get().ifNewVersion()) {
